...
def create_ts_data(data, window_size=5, target_size=3, target_col="OT"):
    df = data.copy()
    ...
    for i in range(0, target_size):
        df["target_{}".format(i+1)] = df[target_col].shift(-i-1)
...
test_dates = data["date"].iloc[int(num_samples * train_ratio):].reset_index(drop=True)
...
for i in range(target_size):
    ...
    plt.plot(dates, predicted, label="Dự báo (Predicted)", color='red', linestyle='--')
